...
export default function OrderItemComponent({
  ...
  function printStatus(status) {
    if (status === DeliveryStatus.confirmed) return "Confirmed";
    else if (status === DeliveryStatus.inTransit) return "In Transit";
    else if (status === DeliveryStatus.pending) return "Pending";
    else if (status === DeliveryStatus.delivered) return "Delivered";
  }
    ...
    <View style={styles.container}>
      ...
      <View style={{ flex: 1 }}>
        ...
        <View style={{ flexDirection: "row", margin: 5 }}>
          ...
          />
          <AppText style={defaultStyles.typography.body.md.regular}>
            {" "}
            {item.color} | Qty = {item.noOfItems}
          </AppText>
        </View>
